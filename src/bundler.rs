...
module.exports = {
  plugins: [
    ...
    fn bundle_with_parcel(&self, input_path: &Path) -> Result<String, Box<dyn std::error::Error>> {
        ...
        if !output.status.success() {
            return Err(
                format!("Parcel failed: {}", String::from_utf8_lossy(&output.stderr)).into(),
            );
        }
  ...
  plugins: [
    new MiniCssExtractPlugin({{
      filename: 'bundle.css'
        ...
        let output = Command::new("npx")
            .args(&["webpack", "--config", config_path.to_str().unwrap()])
            .output()?;
  ...
  plugins: [
    postcss({{
      ...
      plugins: [
        ...
        let output = Command::new("npx")
            .args(&["rollup", "--config", config_path.to_str().unwrap()])
            .output()?;

        if !output.status.success() {
            return Err(
                format!("Rollup failed: {}", String::from_utf8_lossy(&output.stderr)).into(),
            );
        }
